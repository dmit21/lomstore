...
window.onload = function MyPrint() {
  $.ajax({
    ...
    success: function(html1) {
      ...
      document.getElementById('MyPrintZag').innerHTML = 'Возврат залогового имущества № '+arr[1][0][9]+' от '+MySqlDateToStr(arr[1][0][10]);
      ...
      for(i=0;i<n;i++){
        ...
        cell8.innerHTML = arr[1][i][11];
 
        cell9 = row.insertCell(9);
        cell9.innerHTML = arr[1][i][12];
        sum4 = Number(sum4)+Number(arr[1][i][12]);

        cell10 = row.insertCell(10);
        cell10.innerHTML = arr[1][i][7];

        cell11 = row.insertCell(11);
        cell11.innerHTML = arr[1][i][13];
        t.appendChild(row);
      }
      row = t.insertRow(t.rows.length); // Добавляем строку
