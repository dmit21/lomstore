...
try{
  $sql = "SELECT SUM(osum) AS summ FROM sorder WHERE opoint=3 AND  odate='$_SESSION[sdatecur]' AND oorder=1";
...
try{
  $sql = "SELECT SUM(gsumsale) AS summ 
          ...
          WHERE la.pointid = 3 and la.adate = '$_SESSION[sdatecur]' and la.aid = ls.aid and ls.goodid = lz.goodid";
  $result = $pdo->query($sql);
  $row=$result->fetch();
...
try{
  $sql = "SELECT SUM(osum) AS summ FROM sorder WHERE opoint=3 AND odate='$_SESSION[sdatecur]' AND oorder=0";
  $result = $pdo->query($sql);
  $row=$result->fetch();
...
try{
  $sql = "SELECT osid FROM sostatok WHERE pointid=3 AND odate='$_SESSION[sdatecur]' AND oact=1";
  $result = $pdo->query($sql);
  $row=$result->fetch();
  //$i=count($row);
  if ($row['osid']==0) {
