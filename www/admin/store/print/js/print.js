...
window.onload = function MyPrint() {
  $.ajax({
    ...
    success: function(html1) {
      ...
      var cd ="Передача имущества на ответственное хранение по акту разукомплектации № "+nnum+" от " + MyCurrentDate();
      ...
      for (i=0;i<n;i++){
        ...
        cell10.innerHTML = prnarr[i][7];
        sumseb = Number(sumseb)+Number(prnarr[i][7]);

        cell11 = row.insertCell(11);
        cell11.className += "cell11";
        cell11.innerHTML = MyIn(prnarr[i][8]);
        t.appendChild(row);
      }
      row = t.insertRow(t.rows.length); // Добавляем строку
      cell0 = row.insertCell(0);
