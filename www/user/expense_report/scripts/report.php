...
try {
  $sql = "SELECT osum AS summ
          FROM sostatok
          WHERE pointid='$_SESSION[spoint]' AND  odate='$_SESSION[srdate]'
          ORDER BY oact";
...
$cs=$result->rowcount();
  if ($cs!=0) { 
    ...
    // приход
    try{
      $sql = "SELECT SUM(osum) AS summ FROM sorder WHERE opoint='$_SESSION[spoint]' AND  odate='$_SESSION[srdate]' AND oorder=1";
      $result = $pdo->query($sql);
      $row=$result->fetch();
    ...
    // расход
    try{
      $sql = "SELECT SUM(osum) AS summ FROM sorder WHERE opoint='$_SESSION[spoint]' AND odate='$_SESSION[srdate]' AND oorder=0";
      $result = $pdo->query($sql);
      $row=$result->fetch();
    ...
    try{
      $sql = "SELECT SUM(gsumsale) AS summ 
              ...
              WHERE la.pointid = '$_SESSION[spoint]' and la.adate = '$_SESSION[srdate]' and la.aid = ls.aid and ls.goodid = lz.goodid";
      $result = $pdo->query($sql);
      $row=$result->fetch();
      $sumsale=$row['summ'];
    }
    ...
    try{
      $sql = "SELECT SUM(gsumsaleadm) AS summ 
              ...
              WHERE la.pointid = '$_SESSION[spoint]' and la.ndate = '$_SESSION[srdate]' and la.nact = 1 AND la.nid = ls.nid and ls.goodid = lz.goodid";
      $result = $pdo->query($sql);
      $row=$result->fetch();
      $saleopt=$row['summ'];
    }
    ...
    try {
      $sql = "SELECT sum(ncor) AS summ
              ...
              WHERE ls.adateadm = '$mydate' and ln.actid = ls.aid AND ln.nid=lna.nid AND ln.nact = 1 AND ln.pointid = '$_SESSION[spoint]'";
      $result = $pdo->query($sql);
      $row=$result->fetch();
    ...
    try {
      $sql = "SELECT sum(gsum) AS summ
              ...
              WHERE ln.ndate = '$mydate' AND ln.nid=lna.nid and ln.nact = 3 AND ln.pointid = $_SESSION[spoint]'";
      $result = $pdo->query($sql);
      $row=$result->fetch();
    ...
    try {
      $sql = "SELECT osum, odate FROM sostatok WHERE oact=1 AND odate<'$_SESSION[srdate]' AND pointid='$_SESSION[spoint]' ORDER BY odate DESC LIMIT 1";
      $result = $pdo->query($sql);
